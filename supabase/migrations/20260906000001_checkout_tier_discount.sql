-- Tier discounts do not need a manager
--
-- checkout_sale refused any discount from a cashier, including the one the
-- loyalty tier applies by itself - so every sale to a loyalty customer
-- failed at the till. The entitlement is now read from the customer's tier
-- inside the function; anything above it still needs a sign-off.
--
-- Rebuilt from 20260829000002 rather than 20260829000001, so the
-- order_type default that migration added is carried forward.
...
declare
  ...
  v_item jsonb; v_product_id uuid; v_variant_id uuid;
  v_qty numeric; v_unit_price numeric; v_avg_cost numeric;
  v_available numeric; v_inv_id uuid; v_subtotal numeric := 0;
  v_discount_amount numeric; v_vat_percent numeric; v_vat_amount numeric;
  v_grand_total numeric; v_amount_received numeric; v_advance numeric;
  v_change numeric; v_balance_due numeric; v_payment_method text;
  v_is_cod boolean; v_sale public.sales%rowtype; v_ref text;
  v_remaining numeric; v_deduct numeric; v_batch record; v_name text;
begin
  ...
  if not public.can_write_store(p_store_id) then
    raise exception 'Not authorised to sell for store %', p_store_id; end if;
  ...
  -- the cashier granting a favour and needs no sign-off. The entitlement is
  -- read from the customer's tier here rather than trusted from the request.
  ...
  values (
    ...
    coalesce(nullif(p_payment->>'order_type',''), 'walk_in'),
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    set stock_qty = stock_qty - v_qty, updated_at = now() where id = v_inv_id;
    v_remaining := v_qty;
    for v_batch in
