-- =====================================================================
-- submit_sale_return: create a return, including at another branch
-- Repo path: supabase/migrations/20260829000006_submit_sale_return.sql
--
-- sale_returns.store_id is the SELLING store - the branch whose books own
-- the goods - while processed_store_id is where the customer actually
-- walked in. For a cross-store return those differ, and RLS quite correctly
-- refuses a cashier writing a row for a store they do not belong to.
--
-- The rules that matter are enforced here instead:
--   * the caller may only set processed_store_id to their own store
--   * store_id must be the sale's actual store, not whatever was posted
--   * quantities cannot exceed what was bought, minus what came back before
--   * money is recomputed from the original line prices
--
...
create or replace function public.submit_sale_return(
  ...
  p_refund_payment    text default null,
...
begin
  ...
  insert into public.sale_returns (
    return_number, original_sale_id, store_id, processed_store_id,
