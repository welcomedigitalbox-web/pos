-- The per-batch quantity was noise: each row already carries the item's
-- balance, and what a batch adds is its expiry date.

drop function if exists public.warehouse_stock_ledger(text);

-- =====================================================================
-- Warehouse stock ledger, batch by batch (without the per-batch count)
