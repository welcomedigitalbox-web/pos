...
export default function RequestInboxPage() {
  ...
  // Write-offs the sale manager has already passed, waiting on the
  // warehouse to confirm the loss against the books.
  const [tab, setTab] = useState<"requests" | "damage">("requests");
  const [damages, setDamages] = useState<PendingDamage[]>([]);
  const [damageBusy, setDamageBusy] = useState<string | null>(null);
  ...
  async function loadDamages() {
    const { data } = await supabase
      .from("stock_damages")
      .select("id, damage_no, store_id, product_id, variant_id, qty, reason, reported_by, approved_by, created_at")
      .eq("status", "pending")
      .order("created_at", { ascending: false });

    const list = (data as unknown as PendingDamage[]) || [];
    if (list.length) {
      ...
      for (const r of list) r.displayName = names.get(r.product_id) || r.product_id;
    }
    setDamages(list);
  }

  async function confirmDamage(damageNo: string, reject = false) {
    setDamageBusy(damageNo);
    try {
      const { error } = await supabase.rpc("warehouse_confirm_damage", {
        p_damage_no: damageNo,
        p_reject: reject,
        p_reason: reject ? damageRejectReason.trim() || null : null,
      });
      if (error) throw error;
      setDamageRejectId(null);
      setDamageRejectReason("");
  ...
  return (
    <div className="pt-4">
      ...
      <div className="flex gap-1 border-b border-slate-200 mb-4">
        {([["requests", t("nav_stockRequest"), pendingCount],
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-x-auto">
        ...
        <div className="text-sm text-slate-400 py-12 text-center border border-slate-200 rounded-xl">
          {t("requestApproval_empty") || "Nothing waiting"}
        </div>
      ) : (
                ...
                <div>
                  ...
                  {lines[0].approved_by && (
                    <div className="text-xs text-green-700 mt-0.5">
                      {t("stockRequest_approved")}: {lines[0].approved_by}
                    </div>
                  )}
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => confirmDamage(key)} disabled={damageBusy === key}
                    className="px-4 py-1.5 bg-green-600 disabled:bg-slate-300 text-white rounded-lg text-sm font-medium">
                    {damageBusy === key ? "…" : t("stockRequest_approve")}
                  </button>
                  <button
                    onClick={() => { setDamageRejectId(key); setDamageRejectReason(""); }}
                    disabled={damageBusy === key}
              ...
              {damageRejectId === key && (
                <div className="mt-3 flex gap-2">
                  <input autoFocus
                    ...
                    value={damageRejectReason}
                    onChange={(e) => setDamageRejectReason(e.target.value)} />
                  <button onClick={() => confirmDamage(key, true)}
                    disabled={damageBusy === key || !damageRejectReason.trim()}
